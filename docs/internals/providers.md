...
adapter in a child scope. V2 adapter implementations live in
`apps/server/src/orchestration-v2/Adapters/` and conform to
[`ProviderAdapter.ts`](../../apps/server/src/orchestration-v2/ProviderAdapter.ts). Read the driver plus its adapter to see how a specific agent's
...
`system` field. Grok and Antigravity append a separate text block to ACP prompts,
which have no system-message field. Cursor supplies context through its SDK prompt. This does not change the stored user message.
...
The Codex adapter persists completed question items as `user_input_request` turn items and
runtime requests with `responseCapability: { type: "message" }`. Their execution nodes do not
block the run. Web, desktop, and mobile use their normal question panels. The request stays
pending after the turn finishes, the provider exits, or the server restarts.
...
resumes a run, queues behind active work, or steers an active turn when its adapter supports it.
Blocking questions retain the provider's live response path.
