// WorkflowLeaf copy of components/Sidebar.tsx that folds threads sharing a
// worktree into one expandable folder. Kept as a whole-file copy so upstream
// sidebar changes never conflict with the fork; re-copy it when upstream moves
...
import { ProjectEnvironmentBadge } from "~/components/ProjectEnvironmentBadge";
...
import { resolveLocalCheckoutBranchMismatch } from "~/components/BranchToolbar.logic";
import {
  createSidebarCollisionDetection,
  createSidebarSortingStrategy,
  restrictBelowSidebarLabel,
} from "~/components/Sidebar.drag";
import { SidebarDragLifecycle, SidebarPointerSensor } from "~/components/Sidebar.pointer";
import { createSidebarListMotion } from "~/components/Sidebar.motion";
import {
...
import { SidebarContent, SidebarGroup, useSidebar } from "~/components/ui/sidebar";
import { SidebarChromeFooter, SidebarChromeHeader } from "~/components/sidebar/SidebarChrome";
import {
  SidebarHeaderIconButton,
  SidebarThreadHeader,
} from "~/components/sidebar/SidebarThreadHeader";
        ...
        <TooltipTrigger
          render={
            ...
            >
              <ChevronDownIcon
                aria-hidden
                className={cn("size-3.5 shrink-0", !item.expanded && "-rotate-90")}
              />
...
export default function WorktreeSidebar() {
  ...
  const renderListItems = useMemo((): readonly (SidebarListItem | WorktreeHeaderItem)[] => {
    ...
    const rowsOf = (
      ...
      section: SidebarSection,
    ): (SidebarListItem | WorktreeHeaderItem)[] => {
      // Pins are placed one by one on purpose, so they never fold.
      if (section === "pinned") return list.map((thread) => threadItem(thread, section));
      ...
      return entries.flatMap((entry): (SidebarListItem | WorktreeHeaderItem)[] => {
        ...
        return entry.expanded
          ? [header, ...entry.threads.map((thread) => threadItem(thread, section))]
          : [header];
      });
    };
    if (
    ...
    const items: (SidebarListItem | WorktreeHeaderItem)[] = [
