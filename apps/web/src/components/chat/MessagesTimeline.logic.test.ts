...
describe("deriveMessagesTimelineRows", () => {
  ...
  it.each([true, false])(
    ...
    (isWorking) => {
      const runId = RunId.make("run-many-tools");
      ...
      const groupId = "work-group:tool-entry-0";
      const input = {
        timelineEntries,
        isWorking,
        ...
        latestRun: {
          runId,
          status: isWorking ? ("running" as const) : ("completed" as const),
          startedAt: createdAt,
          completedAt: isWorking ? null : createdAt,
        },
