...
For multi-step orchestration, \`t3_code_mode_exec\` accepts a JavaScript async function body: call existing tools as \`t3.<tool_name>(args)\`, use \`Promise.all\` to overlap independent calls, and \`return\` a compact result. Discover callable tool schemas with \`t3_code_mode_describe\`. Await every call. A running result includes an \`executionId\` for \`t3_code_mode_wait\` or \`t3_code_mode_cancel\`; use \`includeCalls=true\` to recover the durable recent call journal. Tool errors reject with \`code\` and \`message\`. Cancellation does not undo accepted commands or stop child tasks. This host exposes orchestration/thread tools and JSON values, without filesystem, network, imports, or timers. Command acceptance is distinct from completed agent work; use the existing task/thread waits for completion. Supply a stable \`clientRequestId\` when submitting code that mutates state. The same key and code retrieves the retained execution, including after restart. Interrupted scripts are never replayed. Never replay a script blindly after failure or after its submission key has been evicted.
